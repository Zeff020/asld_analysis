...
if ( fJpsiVeto )
 ...
 dmass =  D_M; // Dmass is the RooRealVar where we temporarily store the double in D_M
 roomass.add(dmass); // Add the RooRealVar to the roomass RooDataSet containing all the D mass values
      
 }


  RooDataHist* roomasshist = roomass.binnedClone();// Generate the RooDataHist from the RooDataSet
  // New Stuff!
  ...
  model -> fitTo(*roomasshist);
  
  
  RooPlot* yframe = y -> frame();
  roomasshist -> plotOn(yframe);
  model->plotOn(yframe);
