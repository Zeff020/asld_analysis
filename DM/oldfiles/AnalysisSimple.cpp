...
int main(){
  ...
  TH1D* mass = new TH1D("D_Mass","",1000,1750,2100);
...
float Mu_Px = LorentzMuon.Px();    
 ...
 if (( std::fabs(Mu_Px) < (Mu_PT*cosh(Mu_ETA) - 3400.)) && (std::fabs(Mu_Px)< 600. || std::fabs(Mu_Px)> 1100.)) continue;
  ...
  //std::cout<<"Number of times Calibration cuts were executed: " <<  CaliCutsOldN<<endl;


  // New Stuff!
  // y defines the dimensions of the histogram!!!
  RooRealVar* y = new RooRealVar("y", "y", 1750, 2100);   
  RooDataHist dataC("dataC", "dataset with y", *y, mass);
  ...
  RooRealVar *sigma1 = new 
  RooRealVar("sigma1","sigma1",5.0,3.0,20.0);         
